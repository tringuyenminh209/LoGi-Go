...
interface WsContextValue {
  ...
  dismissAlert: () => void;
  sendLocation: (lat: number, lng: number, speedKmh?: number, heading?: number) => void;
}
...
export function WsProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        switch (msg.type) {
          case "connected": {
            // Seed initial truck positions sent on connect
          ...
          case "match_proposal": {
            // Refresh proposals list — handled by polling in useMatchProposals
            break;
          }
        }
      } catch {
  ...
  const sendLocation = useCallback(
    (lat: number, lng: number, speedKmh = 0, heading = 0) => {
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        wsRef.current.send(
          JSON.stringify({
            type: "location_update",
            payload: { lat, lng, speed_kmh: speedKmh, heading },
          })
        );
      }
    },
  ...
  return (
    <WsContext.Provider value={{ isConnected, truckPositions, earthquakeAlert, dismissAlert, sendLocation }}>
      {children}
    </WsContext.Provider>
  );
}
